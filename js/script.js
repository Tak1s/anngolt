$(document).ready(function () {
    ...
    function DisplayPhoto(page){//Вывод фотографий на экран
        ...
        for(var link in linkSpase.link[page]){
            if(page === "recent") {
                ...
                $('#content_'+page).append(template_addPhoto({page:page, link: linkSpase.link[page][link]}));
            };
        };
        if(page !== "recent"){
   ...
   function positionPhotos(link){ //Центрирование фото в миниатюре                 
    ...
    function Servis(){//Получение сервисного документа
        Get_xml('http://api-fotki.yandex.ru/api/users/annagoltsberg/', function(result){
    ...
    function Recent(setUrl){//Получение последних 10-и фотографий
